...
export const OrderConfirmation: React.FC = () => {
  ...
  if (!orderDetails || !settings) return null;

  const getItemTotal = (item: OrderDetails['items'][0]) => {
    const basePrice = item.product.price * item.quantity;
    const milkPrice = item.options.milk ? 
      (settings.milkOptions.find(m => m.type === item.options.milk)?.price || 0) * item.quantity : 
      0;
              ...
              <div>
                ...
                {item.options.milk && (
                  <p className="text-sm text-gray-600">
                    ...
                    {settings.milkOptions.find(m => m.type === item.options.milk)?.price > 0 && 
                      ` (+£${settings.milkOptions.find(m => m.type === item.options.milk)?.price.toFixed(2)})`}
                  </p>
                )}
