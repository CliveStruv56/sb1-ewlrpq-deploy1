...
export const CartSummary: React.FC = () => {
  ...
  const getItemTotal = (item: typeof items[0]) => {
    ...
    const optionPrice = item.options.selectedOption ? 
      (options?.find(opt => opt.name === item.options.selectedOption)?.price || 0) * item.quantity : 
      0;
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white shadow-md rounded-lg p-6 mb-8">
        {items.map((item) => (
          <div key={`${item.product.id}-${item.options.selectedOption}`} className="flex justify-between mb-3 pb-3 border-b">
            <div>
              ...
              {item.options.selectedOption && (
                <p className="text-sm text-gray-600">
                  {item.options.selectedOption}
                  {options?.find(opt => opt.name === item.options.selectedOption)?.price > 0 && 
                    ` (+£${options.find(opt => opt.name === item.options.selectedOption)?.price.toFixed(2)})`}
                </p>
              )}
              <div className="flex items-center mt-2">
                <button
                  onClick={() => updateQuantity(item.product.id, item.options.selectedOption, item.quantity - 1)}
